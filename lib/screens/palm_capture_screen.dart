...
class _PalmCaptureScreenState extends State<PalmCaptureScreen> with TickerProviderStateMixin {
  File?        _imageFile;
  ui.Image?    _uiImage;
  img.Image?   _rawImage;
  ...
  void _initROIs(Size sz) {
    if (_rois.length != 3) {
      _rois = [
        Offset(sz.width * 0.25, sz.height * 0.50),
        Offset(sz.width * 0.50, sz.height * 0.45),
        Offset(sz.width * 0.75, sz.height * 0.50),
      ];
    }
  }
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      if (res.isEmpty) { setState(() { _error = 'No valid ROI data.'; _analyzing = false; }); return; }
  ...
  void _saveAndPop() {
    context.read<ScanProvider>().setPalmData(_roiResults);
  ...
  Widget _header() => Container(
    padding: const EdgeInsets.fromLTRB(16, 16, 16, 16), color: white,
    child: Row(children: [
      GestureDetector(
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Palm Color Analysis', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: ink)),
        Text('Mark the palm crease area', style: GoogleFonts.poppins(fontSize: 11, color: slate)),
      ])),
    ]),
  );
  ...
  Widget _uploadZone() => SingleChildScrollView(
    ...
    child: Column(children: [
      ...
      Container(
        ...
        child: Column(children: [
          Container(width: 80, height: 80,
            decoration: BoxDecoration(color: amberLt, borderRadius: BorderRadius.circular(24)),
            child: const Center(child: Text('🤚', style: TextStyle(fontSize: 40)))),
          const SizedBox(height: 20),
          Text('Capture Palm Photo',
            style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w700, color: ink)),
          const SizedBox(height: 8),
          Text('Spread your palm flat and photograph the center.\nBright, even lighting works best.',
      ...
      _tipCard('Open your hand wide under natural light. Camera about 20 cm above your palm — no flash needed.'),
  ...
  Widget _analysisView() => Column(children: [
    if (!_analyzed)
      Container(color: const Color(0xFFFFFBEB),
        ...
        child: Row(children: [
          ...
          Expanded(child: Text('Drag the yellow markers onto the palm crease area',
    ...
    if (_analyzed)
      FadeTransition(opacity: _fadeAnim, child: Container(
        ...
        child: Row(children: [
          for (int i = 0; i < _roiResults.length && i < 3; i++)
  ...
  Widget _tipCard(String text) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(text, style: GoogleFonts.poppins(fontSize: 11, color: const Color(0xFF78350F), height: 1.4))),
    ]),
  );
}
